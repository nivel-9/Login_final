...
import static com.example.login1.DatabaseHelper.TABLE_NAME;
...
public class RegistroSQLite extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_registro_sqlite);
        ...
        editTextDate2.setOnClickListener(v -> {
            ...
            DatePickerDialog datePickerDialog = new DatePickerDialog(RegistroSQLite.this, (view, year1, monthOfYear, dayOfMonth) ->
        ...
        radioGroupGenero.setOnCheckedChangeListener((group, checkedId) -> {
            if (checkedId == R.id.radioButton4) {
                Toast.makeText(RegistroSQLite.this, "Seleccionaste Masculino", Toast.LENGTH_SHORT).show();
            } else if (checkedId == R.id.radioButton3) {
                Toast.makeText(RegistroSQLite.this, "Seleccionaste Femenino", Toast.LENGTH_SHORT).show();
            }
        });
        ...
        checkBoxEducacion.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {
                Toast.makeText(RegistroSQLite.this, "Seleccionaste Educación", Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(RegistroSQLite.this, "Deseleccionaste Educación", Toast.LENGTH_SHORT).show();
            }
        });

        checkBoxTecnologia.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {
                Toast.makeText(RegistroSQLite.this, "Seleccionaste Tecnología", Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(RegistroSQLite.this, "Deseleccionaste Tecnología", Toast.LENGTH_SHORT).show();
            }
        });

        checkBoxComunicacion.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {
                Toast.makeText(RegistroSQLite.this, "Seleccionaste Comunicación", Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(RegistroSQLite.this, "Deseleccionaste Comunicación", Toast.LENGTH_SHORT).show();
            }
        });

        checkBoxSociedad.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {
                Toast.makeText(RegistroSQLite.this, "Seleccionaste Sociedad", Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(RegistroSQLite.this, "Deseleccionaste Sociedad", Toast.LENGTH_SHORT).show();
            }
        });

        activarEnvio.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {
                Toast.makeText(RegistroSQLite.this, "Se enviará una copia de sus respuestas", Toast.LENGTH_SHORT).show();
            }
        });
        ...
        acceso.setOnClickListener(v -> {
            ...
            if (currentUserId == -1) { // Comprobación para insertar o actualizar
                ...
                if (isInserted) {
                    ...
                    startActivity(new Intent(RegistroSQLite.this, MostrarDatosBD.class));
                }
            } else {
